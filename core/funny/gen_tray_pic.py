...
class Scene:
    ...
    def render(self, width=128, height=128, max_depth=2, DOF=True):
        ...
        for x in xrange(width):
            ...
            for y in xrange(height):
                ...
                ray = Ray(origin=self.camera_position,
                          direction=self.camera_look_at + x_shift + y_shift - self.camera_position)
    ...
    def trace(self, ray, max_depth=2, depth=0):
        ...
        return (
               refl_color * obj.reflectivity + obj.color * (1.0 - obj.reflectivity)) * brightness, intersection.distance
