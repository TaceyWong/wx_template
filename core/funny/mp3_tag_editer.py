...
def getAlbumArt(function, artist, name):
    try:
        ...
        url = "http://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=348b4a08fda8a9b3c5a1ab7b2937f071&artist={0}&track={1}&format=json".format(
            artist.lower(), name.lower())
        url = url.replace(' ', '%20')
...
def getData(name):
    url = 'http://tinysong.com/b/{0}?format=json&key=ac948d3bb6b0c253274f5e4d52bf9543'.format(
        name)  # You can use your own API key as well
    urldata = urllib2.urlopen(url)
    data = urldata.read()
