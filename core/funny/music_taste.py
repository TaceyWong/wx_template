...
def get_playlist_songs(playlist_id):
    ...
    api_call = api_call + "&maxResults=" + str(
        MAX_RESULTS) + "&playlistId=" + playlist_id + "&key=" + SIMPLE_DEVELOPER_KEY
    ...
    try:
        ...
        if category == MUSIC_GENRE_CATEGORY:
            ...
            # If musicbrain id available, fetch the tags from musicbrainz site
                ...
                musicbrainz_url = topic['property']['/common/topic/topic_equivalent_webpage']['values'][0][
                                      'value'] + "/tags"
...
def get_genres_for_song_from_wiki(url):
    ...
    for row in rows:
        ...
        if "Genre" in key:
            ...
            genres = [link.text.strip().lower() for link in links if
                      len(link.text.strip().lower().replace("[", "").replace("]", "")) > 2]
            ...
            other_genres = [genre.strip().lower() for genre in genre_string.split(",") if
                            len(genre.strip().lower()) > 2]
